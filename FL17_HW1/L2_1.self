...
L2.6 HTML Forms
Форма — это часть документа, которая используется для сбора информации, введённой пользователями. 
Элемент <form> — это контейнер, внутри которого могут размещаться различные элементы формы. Он может содержать несколько необязательных элементов или атрибутов, например:
url-адрес на веб-сервере, на который будут отправляться данные формы для обработки;
...
Атрибуты элемента <form> содержат информацию, которая относится к форме или всем ее элементам
Значение атрибута представляет собой разделенный пробелами список кодировок символов, которые будут использоваться для отправки формы, например: <form accept-charset="utf-8 iso-8859-1">
Обязательный атрибут, который указывает url обработчика формы, которому передаются данные. Можно указывать абсолютный или относительный адрес. Если значение атрибута не будет указано, то после перезагрузки страницы элементы формы примут значения по умолчанию.
Отвечает за автозаполнение данных поля: может принимать значение on — включено и off — отключено (обычно используется для паролей).
method Задает способ передачи данных формы. Может принимать значения GET и POST. По умолчанию — GET.
novalidator Отключает проверку формы при отправке
name Задает имя формы, которое будет использоваться для доступа к элементам формы. Значением не может быть пустая строка.
Используется для указания MIME-типа данных, отправляемых вместе с формой. MIME-типы определяются отдельным стандартом и задают типы данных, которые могут передаваться в сети Интернет. Атрибут принимает значения:
...
application/x-www-form-urlencoded — тип содержимого по умолчанию
multipart/form-data — используется, если форма отправляет на сервер файл.
Используется совместно с method="post".
target Указывает окно, в которое будет направлена информация, полученная после отправки формы:
_blank — новое окно
_self — тот же фрейм
...
Элемент <input> — это основной элемент формы. Представляет собой поле типизированных данных, обычно с элементом управления формы, позволяющим пользователю редактировать данные. В зависимости от значения атрибута type элемент принимает вид кнопки, поля ввода, переключателя, флажка и др.
type="text" — значение по умолчанию, определяет однострочное текстовое поле. По умолчанию имеет ширину в 20 символов. Это значение можно менять, добавляя атрибуты 
type="number" — элемент управления, который используется для ввода числа с плавающей точкой.
type="email" — элемент управления, используемый для ввода адреса электронной почты. При вводе данных в это поле браузер проверяет является ли введенное значение электронной почтой. А именно, проверяется наличие знака @ и одного символа до и после него.
type="password" — определяет однострочное текстовое поле, предназначенное для ввода пароля. Символы, введенные внутри поля, скрываются.
type="color" — элемент управления, используемый для задания цвета. Определяет палитру цветов.
type="date" — добавляет элемент для выбора даты. В элементе не указывается время.
type="file" — элемент управления, который позволяет пользователю выбрать файл. Часто используется с атрибутом accept, который является подсказкой для ожидаемого типа файла в элементах управления загрузкой файлов.

Элемент <textarea> используется для создания многострочной области, в которую можно вводить текст. Атрибут cols задает ширину поля в символах, атрибут rows задает высоту поля в строках текста. 
Для того чтобы пользователь понимал, какую информацию нужно вводить в элементы формы, добавляют текст с пояснением. Для этой цели используется элемент label или атрибут placeholder элемента input. Парный тег <label> (англ. — метка) представляет собой заголовок элемента <input>. По своему виду метка ничем не отличается от обычного текста, но благодаря ей пользователь может выбрать элемент формы кликом по тексту, расположенному внутри элемента <label>, а не по самому элементу <input>.
<label for="user-name">Логин</label><input type="text" name="user-name" id="user-name">
<label>Пароль    <input type="password" name="password" id="password"> </label>
Для визуального и логического объединения элементов формы используется элемент <fieldset>. Визуально сгруппированные элементы обводятся рамкой и получают внутренние отступы. Рамка растягивается на всю ширину родительского блока. Тег <legend> определяет заголовок для <fieldset> элемента.
Элемент select представляет элемент управления для выбора из набора параметров. Список опций для элемента select состоит из всех дочерних элементов option элемента select. Эти элементы определены заранее и пользователь не может добавлять свои значения.
Для группировки элементов списков используется элемент <optgroup>...</optgroup>, который создает заголовки.
Атрибут multiple является логическим атрибутом. Если атрибут присутствует, то элемент select представляет элемент управления для выбора ноля или более элементов из списка. Если атрибут отсутствует, то элемент select представляет элемент управления для выбора одного элемента из списка элементов.
Атрибут size определяет количество опций для отображения пользователю.
Чтобы получить аналогичный результат, который дает значение атрибута type="image" для элемента input, можно вложить элемент img в button со значением type="submit". В результате создается кликабельная картинка для отправки данных формы на сервер.
Значение по умолчанию атрибута type у элемента input — "text" — то есть текстовое поле.
Значение по умолчанию атрибута type у элемента button — "submit", добавляет кнопку для отправки данных на сервер.
<input type="text" name="user-name" required>
<input type="password" name="password" pattern="^[a-zA-Z0-9]{8,}$">
<input type="text" name="login" value="john.doe.2020" readonly>
<input type="email" name="email" value="john.doe@mail.com">
<label for="product-count">Количество упаковок товара для оптовой покупки</label>
<input type="number" name="product-count" min="3" max="100">
